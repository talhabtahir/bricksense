...
st.set_page_config(
    ...
    page_icon="static/brickicon8.png",  # Path to your favicon fileimport streamlit as st
import tensorflow as tf
from PIL import Image, ImageOps, ExifTags
import numpy as np
...
st.markdown(
    ...
    <style>
        ...
        div[role='radiogroup'] > label > div:checked {
...
upload_option = st.radio("Upload Options", ('Single Image', 'Multiple Images'))

if upload_option == 'Single Image':
    files = st.file_uploader("Please upload an image of the brick wall", type=("jpg", "png", "jpeg", "bmp", "tiff", "webp"))
    if files:
        files = [files]
else:
...
if not files:
    ...
    for file in files:
        try:
            ...
            st.image(image, caption=f"Uploaded Image: {file.name} (Corrected Orientation)", use_column_width=True)
