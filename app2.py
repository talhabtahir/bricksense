...
def predict_tiles_batch(tiles_np, sensitivity=9):
    """
    Run the model on a batch of 224x224 tiles in a single forward pass.
...
def tiled_crack_detection(image_data, sensitivity=9, progress_bar=None, overlap=0.5):
    ...
    overlap controls how much neighbouring tiles share:
      0.0  = no overlap (fastest, original behaviour)
      0.5  = 50% overlap — tiles shift TILE_SIZE//2 each step (recommended)
      0.75 = 75% overlap — finest coverage, ~4x more tiles, slower

    For every pixel covered by multiple overlapping tiles the model P(cracked)
    scores are averaged (score-map fusion), so cracks on tile borders are
    detected reliably even if they only partially fill a single tile.
    """
    MINI_BATCH_SIZE = 64
    ...
    # e.g. overlap=0.5 → step=112px; overlap=0.0 → step=224px (no overlap)
    ...
    # Ensure last tile always reaches bottom/right edge
    ...
    # ── Score-map fusion ───────────────────────────────────────────────────
    # Accumulate P(cracked) for every pixel from all tiles covering it,
    # then divide by coverage count to get an averaged probability map.
    ...
    for tile_idx, (y0, x0, y1, x1) in enumerate(tile_coords):
        p_cracked = float(pred_vecs[tile_idx][1])
    ...
    # The UI outputs use a clean grid; each cell is labelled from fused scores.
    ...
    # Pad original image to exact grid size for drawing
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            ...
            # Classify cell using confidence threshold
            if conf_pct >= CONF_THRESHOLD:
                pred_index = 1   # Cracked
            else:
                # Check "Not a Wall" using the closest sliding-window tile
    ...
    # ── Crop all canvases back to original dimensions ──────────────────────
    ...
    with st.spinner("Processing image..."):
        try:
            ...
            if max(orig_w, orig_h) > MAX_DIMENSION:
                ...
                st.info(
                    f"Image resized from {orig_w}x{orig_h} to {new_w}x{new_h} px "
            ...
            if predictions is not None:
                ...
                if predicted_class == 0:
                    ...
                    st.warning("This is not a brick wall.")
                else:
                ...
                predictions, image_with_border, contours_with_border,                     heatmap_image, contoured_image, overlay_img = import_and_predict(
                ...
                # ══════════════════════════════════════════════════════════
                st.divider()
                st.subheader("Tile-Based Segment Analysis (224 x 224 px tiles) — Sliding Window")
                st.caption(
                    "The image is scanned with overlapping 224 x 224 px tiles. "
                    "Each tile is independently classified and overlapping tiles "
                    "average their P(cracked) scores so cracks on tile borders "
                    "are never missed. "
                    "Green = Normal  |  Red = Cracked  |  Orange = Not a wall"
                )

                # Overlap slider — shown before the Run button
                with st.expander("Sliding Window Settings"):
                    overlap = st.slider(
                    ...
                    st.caption(
                        ...
                        f"Display grid: {math.ceil(est_h/224)} x {math.ceil(est_w/224)} cells"
                    )
                ...
                if run_tiled:
                    ...
                    if summary["cracked"] == 0:
                        ...
                        st.warning(f"Minor cracking detected: {cracked_pct:.1f}% of tiles are cracked.")
                    elif cracked_pct < 60:
                    ...
                    # ── Row 1: colour-coded grid | contours+overlay ────
                    st.write("")
