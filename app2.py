...
def predict_tile(tile_np, sensitivity=9):
    """
    Run the model on a single 224×224 tile.
    Returns (pred_index, pred_vec, conv_output).
    """
    img_tensor = np.expand_dims(tile_np, axis=0) / 255.0
    custom_model = Model(
        inputs=model.inputs,
        outputs=(model.layers[sensitivity].output, model.layers[-1].output),
    )
    conv_output, pred_vec = custom_model.predict(img_tensor, verbose=0)
    conv_output = np.squeeze(conv_output)   # (H, W, C)
    pred_index  = int(np.argmax(pred_vec))
    return pred_index, pred_vec, conv_output
...
def tiled_crack_detection(image_data, sensitivity=9, progress_bar=None):
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            ...
            tile_np = padded_img[y0:y1, x0:x1]

            # Predict
            pred_index, pred_vec, conv_output = predict_tile(tile_np, sensitivity)
